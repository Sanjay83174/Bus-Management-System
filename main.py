...
def check_passwd():
    ...
    for x in mydata:
        if x[0] == adminname and x[1] == pswd:
            flag = True
            messagebox.showinfo(title="HII", message=f"Welcome {adminname}")
            admin_home_page()
    if not flag:
...
# ADMIN FUNCTION 1 : ADD SCHEDULE5
...
# Import required if not already
import tkinter as tk
from tkinter import messagebox
import pickle

# BACK FUNCTION TO MAIN UPDATE CHOICE SCREEN
def back_to_update_choices():
    root5.destroy()
    update_ticket_information_interface()

# BACK FUNCTION TO TICKET NUMBER ENTRY SCREEN
def back_to_ticket_entry():
    root3.destroy()
    update_ticket_information()
...
def update_ticket_information():
    ...
    back_btn = tk.Button(root3, text="Back", width=10, height=1, bg='lightgrey', command=user_home_page)
...
def update_ticket_information_interface():
    ...
    mycursor.execute("select * from tickets where TicketNumber=%s" % tkn)
    mydata = mycursor.fetchall()
    if mydata == []:
        ...
        back_btn = tk.Button(root4, text="Back", width=10, height=1, bg='lightgrey', command=back_to_ticket_entry)
        back_btn.place(x=10, y=10)


# In all update windows below, only 'Back' button is added
...
def new_name():
    ...
    back_btn = tk.Button(root5, text="Back", bg='lightgrey', command=back_to_update_choices)
    back_btn.place(x=10, y=10)
